...
describe("TimeSpan", function()
    ...
    describe("spanCycles", function()
        it("should break multi cycle span into pieces", function()
            ...
            assert.are.equal(spans[1]:beginTime(), Fraction:new(3, 4))
            assert.are.equal(spans[1]:endTime(), Fraction:new(1, 1))
            assert.are.equal(spans[2]:beginTime(), Fraction:new(1, 1))
            assert.are.equal(spans[2]:endTime(), Fraction:new(2, 1))
            assert.are.equal(spans[3]:beginTime(), Fraction:new(2, 1))
            assert.are.equal(spans[3]:endTime(), Fraction:new(3, 1))
            assert.are.equal(spans[4]:beginTime(), Fraction:new(3, 1))
            assert.are.equal(spans[4]:endTime(), Fraction:new(7, 2))
        end)
        it("should preserve subcycle length spans", function()
            ...
            assert.are.equal(spans[1]:beginTime(), Fraction:new(1, 16))
            assert.are.equal(spans[1]:endTime(), Fraction:new(1, 1))
        end)
    end)
